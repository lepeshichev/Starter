...
import ru.sber.demo.repository.CartRepository;
import ru.sber.demo.repository.ClientRepository;
import ru.sber.demo.repository.ProductRepository;
...
public class CartServiceImpl implements CartService {
    ...
    @Override
    public boolean add(long userId, long productId, int amount) {

        if (!productRepository.existsById(productId) || !clientRepository.existsById(userId)) {
            return false;
        }
    ...
    @Override
    public boolean updateAmount(long userId, long productId, int amount) {
        Optional<Cart> cart = cartRepository.findCartByProduct_IdAndClient_Id(productId, userId);
        if (cart.isPresent()) {
